...
const SavedLocationsList = () => {
    ...
    const handleShowList = () => setIsShowList((prev) => !prev);
    ...
    const renderLocations = () => {
        return savedLocations.map((location) => (
            <li key={location} className="w-full relative flex items-center">
                <button
                    onClick={() => {
                        fetchWeatherData(location);
                        handleShowList();
                    }}
                    className="shadow-md p-1.5 w-full rounded-md cursor-pointer hover:bg-amber-200/70 active:scale-95 transition duration-300 truncate pr-6"
    ...
    return (
        <div
            className="group text-gray-500 rounded-sm self-start z-20"
            ref={listRef}
        >
            <button
